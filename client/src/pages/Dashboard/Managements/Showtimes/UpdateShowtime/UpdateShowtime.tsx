...
const UpdateShowtime = () => {
  ...
  const { data: showtime, refetch } = useGetShowtimeQuery(id)
  ...
  const { data: cinemaComplexes, isLoading: isLoadingCimemaComplexes } =
    useGetCinemaComplexesQuery({})

  const [updateApi, { isLoading }] = useUpdateShowtimeMutation()
  ...
  useEffect(() => {
    if (showtime?.data) {
      setSelectedTimeStart([new Date(showtime?.data?.timeStart)])
      setSelectedTimeEnd([new Date(showtime?.data?.timeEnd)])
  ...
  return (
    <div className='relative h-fit w-full rounded-xl border bg-white p-4 shadow-md'>
      ...
      <form onSubmit={handleSubmit(handleUpdate)} className='mx-auto w-[500px]'>
        ...
        <div className='mb-6 flex flex-col'>
          ...
          >
            ...
            {filteredRooms?.map((room: any) => (
              <option value={room?._id} key={room?._id} className='capitalize'>
                {room?.name}
              </option>
            ))}
          </select>
        ...
        <div className='mb-6 flex flex-col'>
          <label htmlFor='timeStart' className='mb-1 font-semibold capitalize'>
          ...
          />
          {errors.timeStart && (
            <div className='text-sm text-[red]'>{errors.timeStart.message}</div>
          )}
        </div>
        <div className='mb-6 flex flex-col'>
